...
<div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-8">
        <div>
            ...
            <p class="text-gray-600 mt-2">Manajemen pengguna dengan role User</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <h3 class="text-sm font-medium text-gray-500">User Baru (7 hari)</h3>
                    <p class="text-2xl font-bold text-gray-900">{{ \App\Models\User::where('role', 'user')->where('created_at', '>=', now()->subDays(7))->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex items-center">
                <div class="p-3 rounded-full bg-purple-100 text-purple-600">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3a4 4 0 118 0v4m-4 12v-6m0 0V7m0 6h6m-6 0H6"></path>
                    </svg>
                </div>
                <div class="ml-4">
                    <h3 class="text-sm font-medium text-gray-500">User Bulan Ini</h3>
                    <p class="text-2xl font-bold text-gray-900">{{ \App\Models\User::where('role', 'user')->whereMonth('created_at', now()->month)->count() }}</p>
                </div>
            </div>
        </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center">
                                ...
                                <p class="mt-1 text-sm text-gray-500">User akan muncul di sini setelah mereka mendaftar.</p>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="space-y-4">
            ...
            @forelse($recentUsers as $recentUser)
                <div class="flex items-center p-3 bg-gray-50 rounded-lg">
                    ...
                    <div class="flex-1">
                        <p class="text-sm font-medium text-gray-900">{{ $recentUser->name }} bergabung</p>
                        <p class="text-xs text-gray-500">{{ $recentUser->created_at->diffForHumans() }}</p>
                    </div>
                    <a href="{{ route('admin.users.show', $recentUser) }}" class="text-blue-600 hover:text-blue-800 text-sm">
                        Lihat →
                    </a>
                </div>
            @empty
